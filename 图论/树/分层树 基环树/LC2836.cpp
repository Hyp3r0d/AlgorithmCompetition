...
public:
  long long getMaxFunctionValue(vector<int>& a, i64 k) {
    i64 n = a.size();
    a.insert(a.begin(), 0);
    vector<std::vector<i64>>g(n + 1); auto rg = g;
    vector<i64> deg(n + 1, 0);
    for(i64 i = 1;i <= n;i ++)a[i] += 1;
    for (i64 i = 1; i <= n; ++i) {
      g[i].push_back(a[i]);
      rg[a[i]].push_back(i);
      deg[a[i]]++;
    ...
    vector<vector<i64>> pts(1);          /* pts[i] = 环内的点 */
    vector<i64> len(1);                        /* len[i] = 环 i 的长度 */
    vector<i64> cycle(n + 1, 0), idx(n + 1, 0);   /* cycle[i] = 点在哪个环上; idx[i] = 点在环的哪个位置 */
    for (i64 i = 1; i <= n; ++i) {
      if (deg[i] and not cycle[i]) {
        pts.push_back({0});
        i64 cur = i;
        do {
          pts.back().push_back(cur - 1);
